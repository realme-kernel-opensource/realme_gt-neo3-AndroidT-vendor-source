...
#define OPLUS_CHECK_LIMIT_TIME  (MM_FB_KEY_RATELIMIT_1H * 5)
...
#define TFA_SPK_F0_DEFAULT_MIN  (250)
...
static void tfa98xx_check_work(struct work_struct *work)
{
	...
	if ((tfa_fb.last_chk_spk != 0)  && ktime_before(ktime_get(), ktime_add_ms(tfa_fb.last_chk_spk, MM_FB_KEY_RATELIMIT_5MIN))) {
		return;
	}
...
void oplus_tfa98xx_check_reg(struct tfa98xx *tfa98xx)
{
	...
	if ((tfa_fb.last_chk_reg != 0)  && ktime_before(ktime_get(), ktime_add_ms(tfa_fb.last_chk_reg, OPLUS_CHECK_LIMIT_TIME))) {
		return;
	}
...
void oplus_tfa98xx_queue_check_work(struct tfa98xx *tfa98xx)
{
	if ((tfa_fb.chk_flag & CHECK_SPEAKER_MASKS) != 0) {
		...
		if ((tfa_fb.last_chk_spk != 0)  && ktime_before(ktime_get(), ktime_add_ms(tfa_fb.last_chk_spk, OPLUS_CHECK_LIMIT_TIME))) {
			return;
		}
