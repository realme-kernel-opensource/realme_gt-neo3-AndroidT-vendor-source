			 ...
			 const struct i2c_device_id *id)
{
	...
	if (IS_ERR_OR_NULL(fsa_priv->regmap)) {
		dev_info(fsa_priv->dev, "%s: Failed to initialize regmap: %d\n",
			__func__, rc);
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
		mm_fb_audio_kevent_named_delay(OPLUS_AUDIO_EVENTID_HEADSET_DET, MM_FB_KEY_RATELIMIT_5MIN, \
			FEEDBACK_DELAY_60S, "payload@@%s: Failed to initialize regmap: %d", __func__, rc);
#endif
		if (!fsa_priv->regmap) {
			rc = -EINVAL;
			goto err_data;
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
		mm_fb_audio_kevent_named_delay(OPLUS_AUDIO_EVENTID_HEADSET_DET, MM_FB_KEY_RATELIMIT_5MIN, \
			FEEDBACK_DELAY_60S, "payload@@%s get tcpc device type_c_port0 fail", __func__);
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
		mm_fb_audio_kevent_named_delay(OPLUS_AUDIO_EVENTID_HEADSET_DET, MM_FB_KEY_RATELIMIT_5MIN, \
			FEEDBACK_DELAY_60S, "payload@@%s: register tcpc notifer fail", __func__);
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
		mm_fb_audio_kevent_named_delay(OPLUS_AUDIO_EVENTID_HEADSET_DET, MM_FB_KEY_RATELIMIT_5MIN, \
			FEEDBACK_DELAY_60S, "payload@@error registering typec mux: %ld", PTR_ERR(fsa_priv->mux));
#endif
...
static int __init fsa4480_init(void)
{
	...
	if (rc) {
		...
		mm_fb_audio_kevent_named_delay(OPLUS_AUDIO_EVENTID_HEADSET_DET, MM_FB_KEY_RATELIMIT_5MIN, \
			FEEDBACK_DELAY_60S, "payload@@fsa4480: Failed to register I2C driver: %d", rc);
	}
#else
	if (rc)
		pr_info("fsa4480: Failed to register I2C driver: %d\n", rc);
